...
export async function uploadImageData(data: string | Buffer, folder: string = 'prompt-directory', filename?: string): Promise<string> {
  try {
    let fileForUpload: string | Buffer;
    ...
    if (typeof data === 'string') {
      if (isDataUrl(data)) {
        ...
        return data; // already hosted
      } else {
        fileForUpload = `data:image/png;base64,${data}`;
      }
    } else {
      const base64 = data.toString('base64');
      fileForUpload = `data:image/png;base64,${base64}`;
    }
    ...
    try {
      ...
      if (typeof fileForUpload === 'string' && isDataUrl(fileForUpload)) {
        const res: any = await cloudinary.uploader.upload(fileForUpload as string, {
          folder,
          public_id: _fileName,
        });
      ...
      if (Buffer.isBuffer(data)) {
        const result = await new Promise((resolve, reject) => {
          cloudinary.uploader.upload_stream({ resource_type: 'auto', folder, public_id: _fileName }, (error, result) => {
            if (error) reject(error);
            else resolve(result);
          }).end(data);
        });
